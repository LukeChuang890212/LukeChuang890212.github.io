...
<div style="background: linear-gradient(135deg, #1a0a0a 0%, #4a1a1a 50%, #8b0000 100%); padding: 30px; border-radius: 12px; margin-bottom: 30px; color: #e0e0e0;">
  ...
  <p style="font-size: 1.1em; line-height: 1.7; color: #f5f5f5;">
    日本が<strong style="color: #ff6b6b;">2回に10得点</strong>を挙げ、試合の勝敗を決定づける重要な1イニングとなった。先発の<strong>鄭浩均（Cheng）</strong>はわずか1.2回で8自責点を喫して降板し、反省すべき点があるとされた。当日の1球ごとのデータと機械学習モデルを使って、客観的に当日の試合展開を検証してみよう。
  </p>
</div>
...
<div style="display: flex; flex-wrap: wrap; gap: 20px; margin: 25px 0;">
  <div style="flex: 1; min-width: 280px; background: #fff3f3; border-left: 5px solid #dc2626; padding: 20px; border-radius: 8px;">
    <h3 style="color: #dc2626; margin-top: 0;">Q1：鄭浩均の投球内容は本当に悪かったのか？</h3>
    <p style="font-size: 1.05em; line-height: 1.6;">2回の崩壊は先発投手一人のせいだったのか？他の投手のパフォーマンスはどうだったのか。</p>
  </div>
  <div style="flex: 1; min-width: 280px; background: #eff6ff; border-left: 5px solid #2563eb; padding: 20px; border-radius: 8px;">
    <h3 style="color: #2563eb; margin-top: 0;">Q2：重要な2回で大谷翔平と対峙した際、鄭浩均にもっとできることはあったか？</h3>
    <p style="font-size: 1.05em; line-height: 1.6;">満塁でカウント2-1という重要な場面で、鄭浩均が投じた外角カーブを大谷翔平が満塁本塁打にした。もっと良い配球選択はあったのか。</p>
  </div>
</div>
...
<div style="background: #fef2f2; border: 2px solid #dc2626; padding: 20px; border-radius: 10px; margin: 20px 0;">
  ...
  <p style="line-height: 1.7;">鄭浩均は繰り返しカウントで後手に回り、打者有利の状況を自ら作り出していた。大谷翔平が打席に入った時点で、すでに満塁の場面であった。大谷翔平を安易に歩かせたくないという判断から、バッテリーはストライクゾーンの9番付近に落ちるカーブで真っ向勝負を選んだが、結果は大谷翔平の満塁本塁打——大量失点を招くこととなった。</p>
</div>
...
<p style="color: #666; font-size: 0.95em;">赤い星は3投手全員の得点打の位置を示す：鄭浩均（1.2回、8自責点）、胡（0.1回、2自責点）、沙（2.0回、3自責点）。鄭浩均が降板した後も失点は止まらなかった。2回序盤の打者への制球問題を除けば、鄭浩均の投球内容自体は、その後に登板したリリーフ陣と比べて明らかに劣っていたわけではない。</p>
...
<p style="color: #666; font-size: 0.95em;">鄭浩均の全54球の球種内訳：チェンジアップ（11球）、カーブ（7球）、フォーシーム（29球）、スライダー（7球）。フォーシームを多用しているが、制球が安定せずボールが多くなった。さらに、ゾーン中央の高めに甘く入る球も少なくなかった。</p>
...
<div style="background: linear-gradient(135deg, #0f172a 0%, #1e3a5f 100%); padding: 25px; border-radius: 12px; margin: 25px 0; color: #e0e0e0;">
  ...
  <p style="font-size: 1.05em; line-height: 1.7; color: #f5f5f5;">
    大谷翔平の<strong>2025年 MLBシーズン</strong>の全打撃データを用い、まず<strong>カーネル密度推定（KDE）</strong>で大谷翔平の各球種に対するスイングパターンを分析した。次に<strong>Random Forest</strong>と<strong>Gradient Boosting Machine（GBM）</strong>モデルを構築し、<strong>P(swing | pitch)</strong>と<strong>P(hit | swing, pitch)</strong>を個別に予測した。ここで「pitch」とは投球の完全なプロファイル（位置、球種、球速、回転数、変化量）を意味する。両者を掛け合わせた<strong>P(hit | pitch) = P(swing | pitch) × P(hit | swing, pitch)</strong>により、あの満塁本塁打の投球の総合的な危険度を定量化した。最後に、モデルを活用してあの場面での最適な配球戦略を提案する。
  </p>
</div>
...
<p style="color: #666; font-size: 0.95em;">大谷翔平の2025年MLB打撃データに基づくKDE推定値——<em>投球位置のみ</em>を条件としている。各行は球種カテゴリで、上から順に：全体（球種不問）、速球系（フォーシーム・ツーシーム・カッターなど）、変化球系（スライダー・カーブなど）、緩急系（チェンジアップ・スプリットなど）。各列はそれぞれ異なる確率の構成要素を表す。<strong>左：P(swing | loc)</strong>——大谷翔平がスイングする位置。<strong>中央：P(hit | swing, loc)</strong>——スイング後に安打が生まれやすい位置。<strong>右：P(hit | loc)</strong>——各位置の総合危険度。赤い十字は鄭浩均が投じた満塁本塁打のカーブ位置（pX=−0.46, pZ=2.16）を示す。</p>
...
<p style="color: #666; font-size: 0.95em; margin-bottom: 5px;">KDE分析から、あの満塁本塁打の球は球速・回転数・変化量を考慮せず投球位置だけで判断しても、すでにかなり優れた球であったことが分かった。次に、あの満塁本塁打の球が実際に打たれる確率をより正確に評価するため、完全な投球プロファイル——位置、球速、回転数、変化量、カウント——を予測変数としてRFとGBMモデルに入力する：</p>
...
<div style="background: #fefce8; border-left: 5px solid #ca8a04; padding: 20px; border-radius: 8px; margin: 20px 0;">
  <h4 style="color: #92400e; margin-top: 0;">投球の質を考慮しても、あの満塁本塁打の球はやはり良い球だった</h4>
  <p style="line-height: 1.7; margin-bottom: 12px;">SHAP分析により、外角低めの投球位置以外でP(hit | swing, pitch)を低く抑えた主要因が明らかになった：</p>
...
<div style="display: flex; flex-wrap: wrap; gap: 20px; margin: 25px 0;">
  ...
  <div style="flex: 1; min-width: 250px; background: #fefce8; border-left: 5px solid #ca8a04; padding: 20px; border-radius: 8px;">
    <h3 style="color: #ca8a04; margin-top: 0;">鄭浩均は勇敢な投手だ</h3>
    <p style="font-size: 1.05em; line-height: 1.6;">満塁の場面で、鄭浩均は<strong>真っ向勝負を選択した</strong>——2025年ワールドシリーズで同様の場面で大谷翔平を敬遠したブルージェイズとは対照的である。モデルの分析結果は、この判断が勇敢であると同時に理にかなったものであったことを裏付けている。</p>
  </div>
</div>

### 最適配球戦略：鄭浩均はどこを狙うべきだったか？

<p style="color: #666; font-size: 0.95em; margin-bottom: 5px;">鄭浩均のあのカーブは P(hit | pitch) ≈ 9% と、すでに低い値であった。配球戦略の問いに答えるには、鄭浩均の投球プロファイル——各球種の平均球速・回転数・変化量——を固定し、モデルを使って各球種の異なる<strong>投球位置</strong>でのスイング確率およびスイング後の安打確率、すなわちP(swing | Cheng's profile, loc)とP(hit | swing, Cheng's profile, loc)を算出し、より良い配球戦略を探る。</p> 当時のカウントが2-1であり、それ以前に多くの四球を出していたことを考えると、これ以上ボールを投げることは避けるべきであった。そのため、以下の分析ではストライクゾーン内の配球戦略のみを検討する。
...
<p style="color: #666; font-size: 0.95em;">上図はGBMとRFの両モデルが<strong>鄭浩均の各球種の平均投球プロファイル</strong>を入力として、P(swing | Cheng's profile, loc)、P(hit | swing, Cheng's profile, loc)、P(hit | Cheng's profile, loc)について出した平均予測値である。各行は鄭浩均の4球種（フォーシーム、カーブ、チェンジアップ、スライダー）に対応し、各列はそれぞれ異なる確率に対応する。<strong>左：P(swing | Cheng's profile, loc)</strong>——鄭浩均の各球種に対して大谷翔平がスイングする位置。<strong>中央：P(hit | swing, Cheng's profile, loc)</strong>——スイング後に安打が最も生まれやすい位置。<strong>右：P(hit | Cheng's profile, loc)</strong>——各位置の総合安打確率（総合危険度）。赤い十字は実際に満塁本塁打のカーブを投じた位置、青い星は各球種の最適位置（最小P(hit | Cheng's profile, loc)）を示す。</p>
...
<div style="background: #1a1a2e; padding: 25px; border-radius: 12px; margin: 25px 0; text-align: center;">
  ...
  <p style="color: #888; font-size: 0.85em; margin-top: 15px;">loc* = 青い星（最適位置）。</p>
</div>
...
<div style="background: #f8fafc; border: 2px solid #334155; padding: 25px; border-radius: 10px; margin: 20px 0;">
  <ol style="line-height: 2; font-size: 1.05em;">
    <li><strong>カウントで後手に回ったことが主な問題であった</strong>——鄭浩均は繰り返しカウントで不利になり、大谷翔平の打席前に満塁の状況を招いた。崩壊は一連のカウント劣勢が積み重なった結果であった。</li>
    <li><strong>しかしあの満塁本塁打の球は失投ではなかった</strong>——RF/GBMモデルによれば P(swing | pitch) ≈ 71%、P(hit | swing, pitch) ≈ 13%、P(hit | pitch) ≈ 9%。カーブの高回転数（2,483 rpm）、大きな縦変化（−7.09 in.）、約20 mphの球速差がいずれも安打確率を効果的に抑えていた。鄭浩均は敬遠ではなく勝負を選び、勇気を見せた。</li>
    <li><strong>わずかながら改善の余地はあった</strong>——当日の鄭浩均の状態を考えると、カーブの最適位置（青い星）ならP(hit | Cheng's CU, loc)を9%から4.8%に下げられた。ただし、低め外角フォーシーム（3.6%）や低め内角チェンジアップ（4.1%）のほうが多少安全だったかもしれない。</li>
    <li><strong>鄭浩均の降板後も出血は止まらなかった</strong>——後続のリリーフ陣も自責点を記録しており、鄭浩均の全体的な投球内容が彼らより劣っていたわけではなかった。</li>
  </ol>
</div>
...
<div style="background: #f8f9fa; border: 1px solid #dee2e6; padding: 20px; border-radius: 8px; margin-top: 25px; font-style: italic; color: #555;">
  野球は確率の勝負の連続だ。高回転、鋭い変化、20 mphの球速差を備え、安打確率P(hit | pitch)がわずか約9%の投球——それが満塁本塁打になるとは……大谷翔平は本当に凄すぎる、としか言いようがない。
</div>
