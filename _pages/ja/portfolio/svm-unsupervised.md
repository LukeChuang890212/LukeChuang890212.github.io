...
<div style="background: linear-gradient(135deg, #4c1d95 0%, #6d28d9 50%, #8b5cf6 100%); padding: 30px; border-radius: 12px; margin-bottom: 30px; color: #e0e0e0;">
  <h2 style="color: #c4b5fd; margin-top: 0; font-size: 1.6em;">10,000次元空間における欠陥検出</h2>
  <p style="font-size: 1.1em; line-height: 1.7; color: #f5f5f5;">
    生画像を大規模特徴ベクトルに展開し、<strong>PCA</strong> で次元削減、<strong>t-SNE</strong> で可視化、<strong>SVM</strong> で分類——鋳造欠陥検出のためのピクセルから予測までの完全パイプライン。
  </p>
</div>
...
<div style="display: flex; flex-wrap: wrap; gap: 20px; margin: 20px 0;">
  <div style="flex: 1; min-width: 280px; background: #f5f3ff; border-left: 5px solid #8b5cf6; padding: 20px; border-radius: 8px;">
    ...
    <p>高次元データセットを単位分散に標準化。次元削減のために <strong>PCA</strong> を適用。スケーリング済みデータ行列に対して固有値分解を実施。</p>
  </div>
  <div style="flex: 1; min-width: 280px; background: #fdf4ff; border-left: 5px solid #a855f7; padding: 20px; border-radius: 8px;">
    <h3 style="color: #701a75; margin-top: 0;">鋳造欠陥検出</h3>
    <p><strong>100x100 ピクセル</strong>のグレースケール鋳造製品画像。各画像を 10,000 次元の特徴ベクトルに展開。目的：欠陥あり vs. 欠陥なしの分類。</p>
  </div>
</div>

## 探索的データ分析（EDA）

- **スクリープロット**と累積寄与率プロットにより主成分数の選択を決定。
- **t-SNE 埋め込み**（perplexity 3 および 7）が高次元空間における自然なクラスター構造を明示。
- 画像データの**ヒートマップ**が欠陥品と良品を区別する空間パターンを示した。
...
<div style="display: grid; grid-template-columns: repeat(auto-fit, minmax(200px, 1fr)); gap: 15px; margin: 20px 0;">
  ...
  <div style="background: #f5f3ff; border: 2px solid #8b5cf6; padding: 15px; border-radius: 10px; text-align: center;">
    ...
    <p style="margin: 8px 0 0; font-size: 0.9em; color: #666;">CV チューニングされたカーネルとコスト</p>
  </div>
  <div style="background: #f5f3ff; border: 2px solid #8b5cf6; padding: 15px; border-radius: 10px; text-align: center;">
  ...
  <div style="background: #f5f3ff; border: 2px solid #8b5cf6; padding: 15px; border-radius: 10px; text-align: center;">
    ...
    <p style="margin: 8px 0 0; font-size: 0.9em; color: #666;">BIC ベースの選択</p>
  </div>
  <div style="background: #f5f3ff; border: 2px solid #8b5cf6; padding: 15px; border-radius: 10px; text-align: center;">
...
<div style="background: #1a1a2e; padding: 25px; border-radius: 12px; margin: 20px 0; text-align: center;">
  ...
  <div style="display: flex; justify-content: center; gap: 40px; margin-top: 20px; flex-wrap: wrap;">
    ...
    <div><span style="font-size: 1.8em; font-weight: bold; color: #7fecb3;">SVM</span><br><span style="color: #aaa;">最良分類器</span></div>
    <div><span style="font-size: 1.8em; font-weight: bold; color: #7fecb3;">t-SNE</span><br><span style="color: #aaa;">明確なクラスター分離</span></div>
  </div>
</div>

- PCA は識別力を保持しつつ効果的に次元を削減した。
- チューニングされたカーネルを持つ **SVM** が優れた欠陥検出性能を達成。
- t-SNE は明確に分離されたクラスターを確認——自動欠陥検出の実現可能性を裏付けた。
