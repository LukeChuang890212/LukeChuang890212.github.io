...
<div style="background: linear-gradient(135deg, #4a1942 0%, #893168 50%, #c94b8c 100%); padding: 30px; border-radius: 12px; margin-bottom: 30px; color: #e0e0e0;">
  ...
  <p style="font-size: 1.1em; line-height: 1.7; color: #f5f5f5;">
    在 Wisconsin Breast Cancer 資料集上進行 <strong>6 種分類方法</strong>的正面比較。從 Logistic Regression 到 KNN——每個分類器都在 683 個腫瘤樣本、9 項細胞特徵上受到極限考驗。
  </p>
</div>

## 資料來源與清理

**Wisconsin Breast Cancer Data**（UCI / R `mlbench`）：699 筆觀測值，包含 9 個序數型預測因子（描述細胞特徵）和 1 個二元目標變數（良性 vs. 惡性）。移除 16 筆含缺失值的觀測。應用 90/10 的訓練-測試分割，並以密度方法進行驗證。

## 探索性資料分析（EDA）

- 所有預測因子均呈強烈右偏分佈，雙峰模式反映兩類結構。
- 配對散佈圖顯示良性與惡性病例具有明顯不同的分佈——所有預測因子都具有鑑別力。
- 預測因子間的高相關性需要謹慎的建模考量。
...
<div style="display: grid; grid-template-columns: repeat(auto-fit, minmax(200px, 1fr)); gap: 15px; margin: 20px 0;">
  ...
  <div style="background: #fdf2f8; border: 2px solid #c94b8c; padding: 15px; border-radius: 10px; text-align: center;">
    ...
    <p style="margin: 8px 0 0; font-size: 0.9em; color: #666;">不等共變異數矩陣</p>
  </div>
  <div style="background: #fdf2f8; border: 2px solid #c94b8c; padding: 15px; border-radius: 10px; text-align: center;">
...
<div style="background: #1a1a2e; padding: 25px; border-radius: 12px; margin: 20px 0; text-align: center;">
  ...
  <p style="color: #aaa; font-size: 1.1em; margin-top: 5px;">所有分類器——證實資料具有強可分離性</p>
  <div style="display: flex; justify-content: center; gap: 40px; margin-top: 20px; flex-wrap: wrap;">
    ...
    <div><span style="font-size: 1.8em; font-weight: bold; color: #7fecb3;">3</span><br><span style="color: #aaa;">個最重要預測因子</span></div>
    <div><span style="font-size: 1.8em; font-weight: bold; color: #7fecb3;">683</span><br><span style="color: #aaa;">腫瘤樣本</span></div>
  </div>
</div>

- **Cell.size**、**Bare.nuclei** 和 **Bl.cromatin** 在各方法中被識別為最具影響力的預測因子。
- QDA 和無母數 Naive Bayes 透過放寬分佈假設而略有改善。
- KNN 在最佳化 k 值下表現具競爭力，但可解釋性較低。
