...
<div style="background: linear-gradient(135deg, #78350f 0%, #b45309 50%, #f59e0b 100%); padding: 30px; border-radius: 12px; margin-bottom: 30px; color: #e0e0e0;">
  <h2 style="color: #fef3c7; margin-top: 0; font-size: 1.6em;">是什麼讓一座城市昂貴？數據驅動的答案</h2>
  <p style="font-size: 1.1em; line-height: 1.7; color: #f5f5f5;">
    全球各城市的 <strong>55 個支出變數</strong> — 透過 PCA、因素分析與集群分析解構全球生活成本差異背後的隱藏結構。
  </p>
</div>
...
缺失資料透過 **Multiple Imputation** 處理。分析前對資料進行標準化。

## 探索性資料分析 (EDA)

- 各城市間住房與教育成本差異極大。
- 強烈的**領域內相關性**（食品項目彼此相關）及中等的跨領域相關性。
- 分佈分析引導了標準化決策。
...
<div style="display: grid; grid-template-columns: repeat(auto-fit, minmax(200px, 1fr)); gap: 15px; margin: 20px 0;">
  <div style="background: #fffbeb; border: 2px solid #f59e0b; padding: 15px; border-radius: 10px; text-align: center;">
    ...
    <p style="margin: 8px 0 0; font-size: 0.9em; color: #666;">主要變異維度</p>
  </div>
  <div style="background: #fffbeb; border: 2px solid #f59e0b; padding: 15px; border-radius: 10px; text-align: center;">
    <strong style="color: #92400e; font-size: 1.1em;">Factor Analysis</strong>
    <p style="margin: 8px 0 0; font-size: 0.9em; color: #666;">潛在消費模式</p>
  </div>
  <div style="background: #fffbeb; border: 2px solid #f59e0b; padding: 15px; border-radius: 10px; text-align: center;">
    <strong style="color: #92400e; font-size: 1.1em;">Cluster Analysis</strong>
    <p style="margin: 8px 0 0; font-size: 0.9em; color: #666;">依成本特徵分群城市</p>
  </div>
  <div style="background: #fffbeb; border: 2px solid #f59e0b; padding: 15px; border-radius: 10px; text-align: center;">
...
<div style="background: #1a1a2e; padding: 25px; border-radius: 12px; margin: 20px 0; text-align: center;">
  <p style="font-size: 2em; font-weight: bold; color: #fbbf24; margin: 0;">55 個變數 → 可解釋的結構</p>
  <p style="color: #aaa; font-size: 1.1em; margin-top: 5px;">PCA、因素分析與集群分析揭示全球生活成本的組成結構</p>
...
- **PC1** 捕捉整體成本水準；後續主成分區分特定模式（例如高住房但中等食品成本）。
- 因素分析與預設的 8 大領域分類一致 — 驗證了領域知識。
- 集群分析辨識出**高成本**、**中等成本**與**低成本**城市群，並包含細緻的子群。
