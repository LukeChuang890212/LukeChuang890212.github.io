...
<div style="background: linear-gradient(135deg, #3b0764 0%, #7c3aed 50%, #a78bfa 100%); padding: 30px; border-radius: 12px; margin-bottom: 30px; color: #e0e0e0;">
  <h2 style="color: #ddd6fe; margin-top: 0; font-size: 1.6em;">統計學的引擎室——從零開始建構</h2>
  <p style="font-size: 1.1em; line-height: 1.7; color: #f5f5f5;">
    親手實作現代統計學的計算核心：<strong>Monte Carlo</strong>、<strong>MCMC</strong>、<strong>EM Algorithm</strong>、<strong>Newton-Raphson</strong>、<strong>Gaussian Process</strong> 與 <strong>Bayesian Optimization</strong>——所有演算法均從頭撰寫，貫穿 7 份報告。
  </p>
</div>
...
<div style="display: grid; grid-template-columns: repeat(auto-fit, minmax(250px, 1fr)); gap: 15px; margin: 20px 0;">
  ...
  <div style="background: #faf5ff; border: 2px solid #7c3aed; padding: 18px; border-radius: 10px;">
    ...
    <p style="margin: 8px 0 0; font-size: 0.9em; color: #666;">解析梯度與 Hessian 矩陣、Line Search、異質變異數模型</p>
  ...
  <div style="background: #faf5ff; border: 2px solid #7c3aed; padding: 18px; border-radius: 10px;">
    ...
    <p style="margin: 8px 0 0; font-size: 0.9em; color: #666;">Gaussian Mixture Models、E-step/M-step、Log-Likelihood 單調收斂</p>
  ...
  <div style="background: #faf5ff; border: 2px solid #7c3aed; padding: 18px; border-radius: 10px;">
    ...
    <p style="margin: 8px 0 0; font-size: 0.9em; color: #666;">Matern 共變異數 MLE、Kriging、Expected Improvement 採集函數</p>
...
<div style="background: #1a1a2e; padding: 25px; border-radius: 12px; margin: 20px 0; text-align: center;">
  ...
  <p style="color: #aaa; font-size: 1.1em; margin-top: 5px;">每個方法都經過手動撰寫、驗證與分析——而非簡單呼叫現成套件</p>
  <div style="display: flex; justify-content: center; gap: 40px; margin-top: 20px; flex-wrap: wrap;">
    <div><span style="font-size: 1.8em; font-weight: bold; color: #7fecb3;">Quadratic</span><br><span style="color: #aaa;">Newton-Raphson 二次收斂</span></div>
    <div><span style="font-size: 1.8em; font-weight: bold; color: #7fecb3;">EM</span><br><span style="color: #aaa;">成功還原真實參數</span></div>
    <div><span style="font-size: 1.8em; font-weight: bold; color: #7fecb3;">GP</span><br><span style="color: #aaa;">校準的不確定性</span></div>
  </div>
</div>

- 選擇合適提議分配的 Importance Sampling 相較於樸素 Monte Carlo 實現了**顯著的變異數縮減**。
- Jarque-Bera 檢定對厚尾分配具有高檢定力，但對對稱的非常態分配檢定力較低。
- EM Algorithm 成功還原了真實混合模型參數，並展現出 Log-Likelihood 的單調收斂性。
- Newton-Raphson 達到了**二次收斂**，結果與 R 的 `optim` 輸出吻合。
- 基於 Matern 共變異數的 GP 迴歸提供了準確的空間預測和校準良好的不確定性區間。
